...
- **MAPE** (Mean Absolute Percentage Error): Relative error (when applicable)
...
- **RMSE difference**: Spatial CV typically shows higher RMSE (more realistic)
- **R² difference**: Spatial CV typically shows lower R² (less optimistic)
- **Magnitude of difference**: Indicates strength of spatial dependence
- **Best method**: Lowest RMSE indicates best performance under constraints
...
# Get coordinates
coords <- cbind(
  x = test_data$longitude,
  y = test_data$latitude
)
...
for (k in k_values) {
  ...
  distances <- spatial_distance(sample_spatial_data, folds, 
                                "longitude", "latitude")
  mean_dist <- mean(sapply(distances$fold_distances, 
                           function(f) f$mean_distance))
...
- Review [spatial cross-validation methods](spatial-cross-validation.html)
- Learn about [spatial leakage detection](spatial-leakage.html)
- Explore the [introduction](introduction.html) for background

## Key Takeaways

1. **Use spatial CV** for geospatial data to get realistic performance estimates
...
5. **Consider trade-offs** between performance and spatial generalization